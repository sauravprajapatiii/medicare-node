...
export const getAppointmentByPatient = async (req, res) => {
  try {
    ...
    console.error("get appointmentbypatient Error:", error);
    return res.status(500).json({
      success: false,
      message: "Sever Error",
    });
...
export const updateAppointmment = async (req, res) => {
  try {
    const { id } = req.params;
    const body = req.body;
    ...
    console.error("updateAppointmment Error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
