...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    if (!strpe)
      return res
        .status(500)
        .json({ success: false, message: "Stripe is misconfigured" });
    let session;
    try {
    ...
    if (session.payment_status !== "paid")
      return res.status(400).json({
        success: false,
        message: "Payment not compeleted",
      });
...
//to get appointment for paitents
export const getServiceAppointmentsByPaitents = async (req, res) => {
  try {
    const clerkUserId = resolveClerkUserId(req);
    ...
    console.error("getServiceAppointmentsByPaitents Error:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
};
export default {
